...
def main():  
    ...
    target_data = transactions[['cl_id', 'target_flag']]
    target_data = target_data.drop_duplicates()
    target_data.reset_index(drop=True, inplace=True)
    
    data = transactions.rename(columns={'cl_id': 'client_id', 'MCC':'small_group', 'amount':'amount_rur'})
    target_data = target_data.rename(columns={'cl_id':'client_id', 'target_flag':'bins'})
